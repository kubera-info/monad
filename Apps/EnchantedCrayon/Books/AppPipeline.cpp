...
#define B3_MODEL    { D3D12_SHADER_VISIBILITY_VERTEX, Registers::CONSTANT_BUFFER::B3 }
		...
		namespace
		{
			...
			/// </summary>
			SetIfThis
				g_shadow_map{ "shadow"_fx },
				g_shadow_map_font{ "shadow_emoji"_fx, "shadow_font"_fx },
				g_metallic{ "metallic_hr"_fx, "metallic_border"_fx },
				g_shadow_hr{ "shadow_hr"_fx, "shadow_border"_fx },
				g_iconic{ "iconic"_fx, "flag"_fx };
		}
			...
			)
		{
			...
			else if (idFromMany = g_shadow_map_font.IfClass(id);
				idFromMany != g_shadow_map_font.cend())
			{
				insert(*idFromMany,
					{
						T1_FONT,
				...
				insert(id,
					{
						...
						B2_SHADOW,
						B3_MODEL,
						{ D3D12_SHADER_VISIBILITY_VERTEX, Registers::CONSTANT_BUFFER::B4 },
						B1_LIGHT_POS
					});
			}
			...
			else if (idFromMany = g_shadow_hr.IfClass(id);
				idFromMany != g_shadow_hr.cend())
			{
				insert(*idFromMany,
					{
						B2_SHADOW,
			...
			else if (idFromMany = g_iconic.IfClass(id);
				idFromMany != g_iconic.cend())
			{
				insert(*idFromMany,
					{
						T0_SHADOW_MAP,
